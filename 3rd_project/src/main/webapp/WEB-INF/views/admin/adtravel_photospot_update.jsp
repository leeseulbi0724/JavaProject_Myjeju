...
$(document).ready(function() {
	...
	$(".write").click(function() {
		/*
		if ($("input[name^=ps_name]").val() == "") {
			alert("스팟명을 입력해주세요");
			$(this).focus();		
		} else if ($("textarea[name^=ps_infor]").val() == "") {
			alert("설명을 입력해주세요");
			$(this).focus();		
		} else if ($("#file").val() == "") {
			alert("파일을 첨부해주세요");
...
<body style="height:900px">
	...
	<div class = "content_setup_faq">
		...
		<section class = "section_setup_faq">
			...
			<form name="admin_travel_spot" action="adtravel_photospot_update_proc.do" method="post" enctype= "multipart/form-data">			
			<table class="table">
		 		<tr>
		 			<th class="title">스팟</th>
		 			<th>
		 				<select name="spot_choice" class="form-control" id="spot_choice">
		 					<option value="photo" selected>포토스팟</option>
		 					<option value="car">차박스팟</option>
		 				</select>
		 			</th>
		 			<th class="title">이름</th>
		 			<th>
		 				<input type="text" class="form-control" placeholder="포토스팟명을 입력해주세요" id="ps_name" name="ps_name" value="${photovo.ps_name}">
		 			</th>
		 		</tr>
		 		<tr>
		 			<th class="title">설명</th>
		 			<th colspan="4">
		 				<textarea class="form-control" placeholder="포토스팟 설명을 입력해주세요" id="ps_info" name="ps_infor" style="height:200px;">${photovo.ps_infor}</textarea>
		 			</th>
		 		</tr>
		 		<tr>
		 		...
		 		<tr>
		 			...
		 			<th colspan="4">
				 		<c:forEach var="photovo"  items="${list}">
			 				<div>
			 					<input type="text" class="form-control"  id="file" readonly value="${photovo.ps_psfile }" name="${photovo.ps_pssfile }">
			 					<button type="button" class="minus">-</button>
			 				</div>
		 					<span class="file_name">${photovo.ps_psfile}</span>		 					
		 					<span class="sfile_name">${photovo.ps_pssfile}</span>
				 		</c:forEach>
		 			</th>		 			
		 		</tr> 		
		 	</table>
