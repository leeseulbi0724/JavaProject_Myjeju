...
<body>
	...
	<div class = "content_setup_faq">
		...
		<section class ="setup_faq_list">
			<table class = "content_layout_setup_faq">
				<tr>
					...
					<th style="wdith:200px"> 아이디 </th>
					<th> 날짜 </th>
				</tr>			 	
				<c:forEach var="vo" items="${list}">
				<tr>
					<td>${vo.rno }</td>
					<td><a href="adboard_content.do?fid=${vo.fid }" class="title">${vo.ftitle }</a></td>
					<td>${vo.id }</td>
					<td>${vo.fdate }</td>
				</tr>
				</c:forEach>				
				<tr>
					...
					<%	
						...
						if(startPage != 1) {
					%>
						<li><a href="adboard.do?pnum=<%= startPage -1 %>&search=<%= search %>&search_text=<%= search_text %>"><span><</span></a></li>
					<%
						} else {
					%>
					...
					%>
						<li><a href="adboard.do?pnum=<%= i %>&search=<%= search %>&search_text=<%= search_text %>" style="color: #000000;"><%= i %></a></li>
					<%
						}
					%>
						<li class="active_page" ><a href="adboard.do?pnum=<%= pageNumber %>&search=<%= search %>&search_text=<%= search_text %>" style="background-color: #337ab7;color: #ffffff;"><%= pageNumber %></a></li>
					<%
					...
					%>
						<li><a href="adboard.do?pnum=<%= startPage + 10 %>&search=<%= search %>&search_text=<%= search_text %>"style ="color: #000000;"><span>></span></a></li>
					<%
						} else {
					%>
						<li><span style="color: gray;">></span></li>
		...
		<section class = "setup_faq_search">
			<form name = "setup_counsel_form" action ="adboard.do" method = "post">
