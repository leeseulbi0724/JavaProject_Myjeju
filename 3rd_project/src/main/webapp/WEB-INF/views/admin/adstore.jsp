...
<body>
	...
	<div class = "content_setup_faq">
		...
		<section class ="setup_faq_list">
			<a href = "adstore_write.do" class = "write">상품등록</a>
			<table class = "content_layout_setup_faq">
				...
				<tr>
					...
					<td>${vo.sid}</td>
					<td>${vo.s_category}</td>
					<td><a href = "adstore_content.do?sid=${vo.sid}">${vo.s_name}</a></td>
					<td>${vo.s_price}</td>
				</tr>
				</c:forEach>
				<tr>
					...
					<%	
						...
						if(startPage != 1) {
					%>
						<li><a href="adstore.do?pnum=<%= startPage -1 %>&search=<%= search %>&search_text=<%= search_text %>"><span><</span></a></li>
					<%
						} else {
					%>
					...
					%>
						<li><a href="adstore.do?pnum=<%= i %>&search=<%= search %>&search_text=<%= search_text %>" style="color: #000000;"><%= i %></a></li>
					<%
						}
					%>
						<li class="active_page" ><a href="adstore.do?pnum=<%= pageNumber %>&search=<%= search %>&search_text=<%= search_text %>" style="background-color: #337ab7;color: #ffffff;"><%= pageNumber %></a></li>
					<%
					...
					%>
						<li><a href="adstore.do?pnum=<%= startPage + 10 %>&search=<%= search %>&search_text=<%= search_text %>"style="color: #000000;"><span>></span></a></li>
					<%
						} else {
					%>
						<li><span style="color: gray;">></span></li>
		...
		<section class = "setup_faq_search">
			<form name = "setup_counsel_form" action ="adstore.do" method = "post">
				<select class = "search" name = "search" style = "width: 70px; height: 27px">
					<option value = "s_category">카테고리</option>
					<option value = "s_name">상품명</option>
				</select>
